...
public class Ex03_Stream {
	public static void main(String[] args) {
		...
		//4. min()
		//최소값을 Optional로 반환
		Optional<String> min = Stream.of("dog","cat","zebra").min(Comparator.naturalOrder());
		
		System.out.println(min.get());
		
		//5.max
		//최대값을 Optional로 반환
				Optional<String> max = Stream.of("dog","cat","zebra").max(Comparator.naturalOrder());
				
				System.out.println(max.get());
				
				//6. anyMatch()
				//하나라도 조건이 ㅁ나족하는지
				boolean hasShort = Stream.of("Java", "Go","Phython")
											.anyMatch(s -> s.length() <=2);
				
				//7. allMatch()
				//모든 요소가 조건을 만족하는지 판별
				boolean allLong = Stream.of("Java","Kotlin").allMatch(s -> s.length() >= 4);
